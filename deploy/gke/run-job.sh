...
PROJECT_ID=${PROJECT:?"PROJECT environment variable must be set"}
...
sed -e "s|PROJECT_ID|$PROJECT_ID|g" -e "s|IMAGE_TAG|$IMAGE_TAG|g" deploy/gke/job-build-index.yaml | kubectl apply -f -
